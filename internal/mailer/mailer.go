...
// Below, declare a new variable with the type embed.FS (embedded file system) to hold
// to hold our email templates. This has a comment directive in the format `//go:embed <path>`
// IMMEDIATELY BEFORE the variable declaration (that is above it).
// this indicates to go that we want to store the contents of the ./templates directory in the templateFS embedded file system variable.
// ↓↓↓
...
// SMTP server) and the sender info for your emails (the name and address you
...
// dynamic data for the template as an interface{} parameter.
...
func (m Mailer) Send(recipient, templateFile string, data interface{}) error {
	...
	// the SetBody() method to set the plain-text body, and the AddAlternative() method to set the HTML body. Its important to note
	// that AddAlternative() should always be called *after* SetBody().
	...
	for i := 0; i <= 3; i++ {
		...
		// Call the DialAndSend() method on the dialer, passing the message to send.
		// This opens a connection to the SMTP server, sends the email, and closes the connection.
		...
		//if it didnt work or there was an error sleep for 5 seconds
